...
int main(int argc, char *argv[]) {
  ...
  int repeat_data = atoi(getenv("RODINIA_REPEAT_DATA"));
  ...
  numRecords = numRecords * repeat_data;
  ...
  if (!quiet) {
    ...
    fprintf(stderr, "Finding the %d closest neighbors.\n",resultsCount);
  }
...
float *OpenClFindNearestNeighbors(
    ...
    if (timing) {
        ...
        fprintf(stderr, "# Records\tWrite(s) [size]\t\tKernel(s)\tRead(s)  [size]\t\tTotal(s)\n");
        fprintf(stderr, "%d        \t",numRecords);
        // Write Buffer
        ...
        fprintf(stderr, "%f [%.2fMB]\t",(float)((eventEnd-eventStart)/1e9),(float)((sizeof(LatLong) * numRecords)/1e6));
        ...
        fprintf(stderr, "%f\t",(float)((eventEnd-eventStart)/1e9));
        totalTime += eventEnd-eventStart;
        // Read Buffer
        ...
        fprintf(stderr, "%f [%.2fMB]\t",(float)((eventEnd-eventStart)/1e9),(float)((sizeof(float) * numRecords)/1e6));
        totalTime += eventEnd-eventStart;
...
int loadData(char *filename,std::vector<Record> &records,std::vector<LatLong> &locations){
	...
	while(!feof(flist)) {
		...
		*/
        ...
        fp = fopen(dbname, "r");
        if(!fp) {
            fprintf(stderr, "error opening a db\n");
            exit(1);
        }
        // read each record
        while(!feof(fp)){
...
void printUsage(){
  fprintf(stderr, "Nearest Neighbor Usage\n");
  fprintf(stderr, "\n");
  fprintf(stderr, "nearestNeighbor [filename] -r [int] -lat [float] -lng [float] [-hqt] [-p [int] -d [int]]\n");
  fprintf(stderr, "\n");
  fprintf(stderr, "example:\n");
  fprintf(stderr, "$ ./nearestNeighbor filelist.txt -r 5 -lat 30 -lng 90\n");
  fprintf(stderr, "\n");
  fprintf(stderr, "filename     the filename that lists the data input files\n");
  fprintf(stderr, "-r [int]     the number of records to return (default: 10)\n");
  fprintf(stderr, "-lat [float] the latitude for nearest neighbors (default: 0)\n");
  fprintf(stderr, "-lng [float] the longitude for nearest neighbors (default: 0)\n");
  fprintf(stderr, "\n");
  fprintf(stderr, "-h, --help   Display the help file\n");
  fprintf(stderr, "-q           Quiet mode. Suppress all text output.\n");
  fprintf(stderr, "-t           Print timing information.\n");
  fprintf(stderr, "\n");
  fprintf(stderr, "-p [int]     Choose the platform (must choose both platform and device)\n");
  fprintf(stderr, "-d [int]     Choose the device (must choose both platform and device)\n");
  fprintf(stderr, "\n");
  fprintf(stderr, "\n");
  fprintf(stderr, "Notes: 1. The filename is required as the first parameter.\n");
  fprintf(stderr, "       2. If you declare either the device or the platform,\n");
  fprintf(stderr, "          you must declare both.\n\n");
}
